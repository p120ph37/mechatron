...
module.exports = function (mechatron, log, assert, waitFor, waitForAsync) {
	...
	return [
		...
		// ── Fuzz: read-only window queries stress loop ──
		{
			name: "window read-only fuzz (200 iterations)",
			functions: [
				"window_getList", "window_isValid", "window_getTitle",
				"window_getBounds", "window_getClient", "window_getProcess",
				"window_getPID", "window_getHandle", "window_isTopMost",
				"window_isBorderless", "window_isMinimized", "window_isMaximized",
				"window_mapToClient", "window_mapToScreen",
			],
			test: async function () {
				for (var i = 0; i < 200; i++) {
					var list = await Window.getList();
					...
					assert(typeof await w.isValid() === "boolean", "rfuzz valid");
					await w.getTitle();
					var b = await w.getBounds();
					assert(b instanceof Bounds, "rfuzz bounds");
					var c = await w.getClient();
					assert(c instanceof Bounds, "rfuzz client");
					assert(typeof await w.getPID() === "number", "rfuzz pid");
					assert(typeof w.getHandle() === "number", "rfuzz handle");
					assert(typeof await w.isTopMost() === "boolean", "rfuzz topmost");
					assert(typeof await w.isBorderless() === "boolean", "rfuzz borderless");
					assert(typeof await w.isMinimized() === "boolean", "rfuzz minimized");
					assert(typeof await w.isMaximized() === "boolean", "rfuzz maximized");
					var mc = await w.mapToClient(b.x + 1, b.y + 1);
					assert(mc instanceof Point, "rfuzz mapToClient");
					var ms = await w.mapToScreen(1, 1);
					assert(ms instanceof Point, "rfuzz mapToScreen");
					assert(typeof await w.getProcess() === "object", "rfuzz process");
				}
			}
		},
		// ── Fuzz: getActive / setActive / isAxEnabled stress loop ──
		{
			name: "window active+ax fuzz (200 iterations)",
			functions: [
				"window_getActive", "window_setActive", "window_isAxEnabled",
				"window_getList",
			],
			test: async function () {
				for (var i = 0; i < 200; i++) {
					var active = await Window.getActive();
					assert(active instanceof Window, "afuzz active");
					assert(typeof await Window.isAxEnabled() === "boolean", "afuzz ax");
					if (i % 20 === 0 && await active.isValid()) {
						await Window.setActive(active);
					}
				}
			}
		},
		// ── Fuzz: setters on valid window (no-crash, idempotent ops) ──
		{
			name: "window setter fuzz (100 iterations)",
			functions: [
				"window_getList", "window_isValid", "window_setMinimized",
				"window_setMaximized", "window_setBounds", "window_setClient",
				"window_setTitle",
			],
			test: async function () {
				var list = await Window.getList();
				if (list.length === 0) return;
				var w = list[0];
				if (!await w.isValid()) return;
				var origBounds = await w.getBounds();
				var origTitle = await w.getTitle();
				for (var i = 0; i < 100; i++) {
					await w.setMinimized(false);
					await w.setMaximized(false);
					await w.setBounds(origBounds.x, origBounds.y, origBounds.w, origBounds.h);
					await w.setClient(origBounds.x, origBounds.y, origBounds.w, origBounds.h);
					if (origTitle) await w.setTitle(origTitle);
				}
			}
		},
	];
