...
module.exports = function (mechatron, log, assert, waitFor, expectOrSkip) {
	...
	function testScreen() {
		...
		var fb = require("../lib/screen/framebuffer");
		...
		// Our rowToArgb 32bpp path reads B,G,R,A and packs to ARGB.
		...
		// red-only: R=0x1F,G=0,B=0 → pix = 0xF800
		src16[0] = 0x00; src16[1] = 0xF8;
		// green-only: R=0,G=0x3F,B=0 → pix = 0x07E0
		...
		fbDv.setUint32(0, 0x11, true);   // fb_id
		fbDv.setUint32(4, 2560, true);   // width
		fbDv.setUint32(8, 1440, true);   // height
		fbDv.setUint32(12, 10240, true); // pitch
		fbDv.setUint32(16, 32, true);    // bpp
		fbDv.setUint32(20, 24, true);    // depth
		fbDv.setUint32(24, 0xABCD, true);// handle
		...
		new DataView(crtcBuf.buffer).setUint32(16, 99, true); // fb_id
		...
		// Availability probes — just verify they return booleans (device
		// presence varies by host).
		...
		// dlopen + captureSelected gate.  Live capture always returns null
		// here because /dev/fb0 and /dev/dri/card0 don't exist in CI.
		...
		if (be === "ffi" && process.platform === "linux") {
			...
			// libc should open on Linux — libc.so.6 is always present.
			assert(libcMod.libc() !== null || libcMod.libcOpenReason() !== null,
				"libc() resolves or reports a reason");
			// captureSelected returns null when the screen mechanism isn't
			// pinned to framebuffer/drm.
			...
			// Direct capture calls: safe to invoke; if the device doesn't
			// exist they fail gracefully.
