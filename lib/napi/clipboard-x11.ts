/**
 * napi clipboard backend (x11 variant) — Linux-only.
 *
 * Loads the mechatron-clipboard-x11.<platform>.node binary out of the
 * @mechatronic/napi-clipboard package.  That binary has libX11.so.6 as a
 * NEEDED entry, so a `require` failure here cleanly indicates an X11-less
 * system; the dispatcher in lib/backend.ts catches the error and falls
 * through to the next backend variant.
 */

import { loadNapi } from "./resolve";

const native = loadNapi("clipboard", "x11");
