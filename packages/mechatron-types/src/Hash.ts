...
export class Hash {
  ...
  constructor(a?: Hash | number | string | Buffer | Uint8Array | ArrayBuffer | number[]) {
    this.result = 0;
    if (a === undefined) return;
    if (a instanceof Hash) {
      this.result = a.result; return;
    }
    if (typeof a === "number") {
      this.result = a; return;
    }
    this.append(a);
  }

  append(data: any): void {
    if (data === undefined) throw new TypeError("Invalid arguments");
  ...
  eq(other: Hash | number): boolean {
    if (other instanceof Hash) return this.result === other.result;
    if (typeof other === "number") return this.result === (other >>> 0);
