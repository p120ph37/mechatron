// Linux x11 clipboard implementation entry point.
//
// Loaded by the `mechatron-clipboard-x11` crate.  Hosts the
// `clipboard_x11` helper module (ICCCM selection background-thread
// implementation against an Xlib display) plus the napi-derive #[napi]
// exports that call into it.
//
...
// ── PNG conversion helpers (used by clipboard_x11 via super::) ────────
...
impl Task for ClearTask {
    type Output = bool;
    type JsValue = bool;
    fn compute(&mut self) -> Result<bool> { Ok(clipboard_x11::x11_clear()) }
...
impl Task for HasTextTask {
    type Output = bool;
    type JsValue = bool;
    fn compute(&mut self) -> Result<bool> { Ok(clipboard_x11::x11_has_text()) }
    fn resolve(&mut self, _env: Env, out: bool) -> Result<bool> { Ok(out) }
}
...
impl Task for GetTextTask {
    type Output = String;
    type JsValue = String;
    fn compute(&mut self) -> Result<String> { Ok(clipboard_x11::x11_get_text()) }
...
impl Task for SetTextTask {
    type Output = bool;
    type JsValue = bool;
    fn compute(&mut self) -> Result<bool> { Ok(clipboard_x11::x11_set_text(&self.text)) }
    fn resolve(&mut self, _env: Env, out: bool) -> Result<bool> { Ok(out) }
}
...
impl Task for HasImageTask {
    type Output = bool;
    type JsValue = bool;
    fn compute(&mut self) -> Result<bool> { Ok(clipboard_x11::x11_has_image()) }
    fn resolve(&mut self, _env: Env, out: bool) -> Result<bool> { Ok(out) }
}
...
impl Task for GetImageTask {
    ...
    fn compute(&mut self) -> Result<Option<(u32, u32, Vec<u32>)>> {
        Ok(clipboard_x11::x11_get_image())
    }
...
impl Task for SetImageTask {
    ...
    fn compute(&mut self) -> Result<bool> {
        Ok(clipboard_x11::x11_set_image(self.width, self.height, &self.data))
    }
    fn resolve(&mut self, _env: Env, out: bool) -> Result<bool> { Ok(out) }
}
...
impl Task for GetSequenceTask {
    type Output = f64;
    type JsValue = f64;
    fn compute(&mut self) -> Result<f64> { Ok(clipboard_x11::x11_get_sequence()) }
