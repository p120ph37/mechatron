...
function createBridgeBackend(addon) {
  const kb = new addon.Keyboard;
  const ms = new addon.Mouse;
  return {
    keyboard_click(keycode) {
      kb.click(keycode);
    },
    ...
    keyboard_compile(keys) {
      return addon.Keyboard.compile(keys);
    },
    keyboard_getState() {
      return addon.Keyboard.getState();
    },
    keyboard_getKeyState(keycode) {
      return addon.Keyboard.getState(keycode);
    },
    mouse_click(button) {
      ms.click(button);
    },
    ...
    mouse_scrollH(amount) {
      ms.scrollH(amount);
    },
    mouse_scrollV(amount) {
      ms.scrollV(amount);
    },
    mouse_getPos() {
      const p = addon.Mouse.getPos();
      return { x: p.x, y: p.y };
    },
    mouse_setPos(x, y) {
      addon.Mouse.setPos(x, y);
    },
    mouse_getState() {
      return addon.Mouse.getState();
    },
    mouse_getButtonState(button) {
      return addon.Mouse.getState(button);
    },
    clipboard_clear() {
      return addon.Clipboard.clear();
    },
    clipboard_hasText() {
      return addon.Clipboard.hasText();
    },
    clipboard_getText() {
      return addon.Clipboard.getText();
    },
    clipboard_setText(text) {
      return addon.Clipboard.setText(text);
    },
    clipboard_hasImage() {
      return addon.Clipboard.hasImage();
    },
    clipboard_getImage() {
      const img = new addon.Image;
      if (!addon.Clipboard.getImage(img))
        return null;
      return { width: img.getWidth(), height: img.getHeight(), data: img.getData() };
    },
    clipboard_setImage(width, height, data) {
      const img = new addon.Image;
    ...
    clipboard_getSequence() {
      return addon.Clipboard.getSequence();
    },
    screen_synchronize() {
      if (!addon.Screen.synchronize())
        return null;
    ...
    screen_grabScreen(x, y, w, h, windowHandle) {
      const img = new addon.Image;
    ...
    screen_isCompositing() {
      return addon.Screen.isCompositing();
    },
    screen_setCompositing(enabled) {
      addon.Screen.setCompositing(enabled);
    },
    ...
    window_isValid(handle) {
      return new addon.Window(handle).isValid();
    },
    window_close(handle) {
      new addon.Window(handle).close();
    },
    window_isTopMost(handle) {
      return new addon.Window(handle).isTopMost();
    },
    window_isBorderless(handle) {
      return new addon.Window(handle).isBorderless();
    },
    window_isMinimized(handle) {
      return new addon.Window(handle).isMinimized();
    },
    window_isMaximized(handle) {
      return new addon.Window(handle).isMaximized();
    },
    window_setTopMost(handle, topMost) {
      new addon.Window(handle).setTopMost(topMost);
    },
    window_setBorderless(handle, borderless) {
      new addon.Window(handle).setBorderless(borderless);
    },
    window_setMinimized(handle, minimized) {
      new addon.Window(handle).setMinimized(minimized);
    },
    window_setMaximized(handle, maximized) {
      new addon.Window(handle).setMaximized(maximized);
    },
    window_getProcess(handle) {
      return new addon.Window(handle).getProcess().getPID();
    },
    window_getPID(handle) {
      return new addon.Window(handle).getPID();
    },
    window_getHandle(handle) {
      return handle;
    },
    window_setHandle(handle, newHandle) {
      return new addon.Window(handle).setHandle(newHandle);
    },
    window_getTitle(handle) {
      return new addon.Window(handle).getTitle();
    },
    window_setTitle(handle, title) {
      new addon.Window(handle).setTitle(title);
    },
    window_getBounds(handle) {
      const b = new addon.Window(handle).getBounds();
      return { x: b.x, y: b.y, w: b.w, h: b.h };
    },
    window_setBounds(handle, x, y, w, h) {
      new addon.Window(handle).setBounds(x, y, w, h);
    },
    window_getClient(handle) {
      const b = new addon.Window(handle).getClient();
      return { x: b.x, y: b.y, w: b.w, h: b.h };
    },
    window_setClient(handle, x, y, w, h) {
      new addon.Window(handle).setClient(x, y, w, h);
    },
    window_mapToClient(handle, x, y) {
      const p = new addon.Window(handle).mapToClient(x, y);
      return { x: p.x, y: p.y };
    },
    window_mapToScreen(handle, x, y) {
      const p = new addon.Window(handle).mapToScreen(x, y);
      return { x: p.x, y: p.y };
    },
    window_getList(regex) {
      const list = regex !== undefined ? addon.Window.getList(regex) : addon.Window.getList();
      return list.map((w) => w.getHandle());
    },
    window_getActive() {
      return addon.Window.getActive().getHandle();
    },
    window_setActive(handle) {
      addon.Window.setActive(new addon.Window(handle));
    },
    window_isAxEnabled(prompt) {
      return addon.Window.isAxEnabled(prompt);
    },
    process_open(pid) {
    ...
    process_close(pid) {
      new addon.Process(pid).close();
    },
    process_isValid(pid) {
      return new addon.Process(pid).isValid();
    },
    process_is64Bit(pid) {
      return new addon.Process(pid).is64Bit();
    },
    process_isDebugged(pid) {
      return new addon.Process(pid).isDebugged();
    },
    process_getPID(pid) {
      return pid;
    },
    process_getName(pid) {
      return new addon.Process(pid).getName();
    },
    process_getPath(pid) {
      return new addon.Process(pid).getPath();
    },
    process_exit(pid) {
      new addon.Process(pid).exit();
    },
    process_kill(pid) {
      new addon.Process(pid).kill();
    },
    process_hasExited(pid) {
      return new addon.Process(pid).hasExited();
    },
    process_getModules(pid, regex) {
      const p = new addon.Process(pid);
    ...
    process_getWindows(pid, regex) {
      const p = new addon.Process(pid);
      const wins = regex !== undefined ? p.getWindows(regex) : p.getWindows();
      return wins.map((w) => w.getHandle());
    },
    process_getList(regex) {
      const list = regex !== undefined ? addon.Process.getList(regex) : addon.Process.getList();
      return list.map((p) => p.getPID());
    },
    process_getCurrent() {
      return addon.Process.getCurrent().getPID();
    },
    process_isSys64Bit() {
      return addon.Process.isSys64Bit();
    },
    process_getSegments(pid, base) {
      const p = new addon.Process(pid);
      return addon.Process._getSegments(p, base);
    },
    memory_isValid(pid) {
      return new addon.Memory(new addon.Process(pid)).isValid();
    },
    memory_getRegion(pid, address) {
    ...
    memory_getRegions(pid, start, stop) {
      const m = new addon.Memory(new addon.Process(pid));
    ...
    memory_setAccess(pid, regionStart, readable, writable, executable) {
      const m = new addon.Memory(new addon.Process(pid));
      const r = m.getRegion(regionStart);
      return m.setAccess(r, readable, writable, executable);
    },
    memory_setAccessFlags(pid, regionStart, flags) {
      const m = new addon.Memory(new addon.Process(pid));
      const r = m.getRegion(regionStart);
      return m.setAccess(r, flags);
    },
    memory_getPtrSize(pid) {
      return new addon.Memory(new addon.Process(pid)).getPtrSize();
    },
    memory_getMinAddress(pid) {
      return new addon.Memory(new addon.Process(pid)).getMinAddress();
    },
    memory_getMaxAddress(pid) {
      return new addon.Memory(new addon.Process(pid)).getMaxAddress();
    },
    memory_getPageSize(pid) {
      return new addon.Memory(new addon.Process(pid)).getPageSize();
    },
    memory_find(pid, pattern, start, stop, limit, flags) {
      const m = new addon.Memory(new addon.Process(pid));
      return m.find(pattern, start, stop, limit, flags);
    },
    memory_readData(pid, address, length, flags) {
      const m = new addon.Memory(new addon.Process(pid));
      const buf = Buffer.alloc(length);
      const read = m.readData(address, buf, length, flags);
      return read > 0 ? buf : null;
    },
    memory_writeData(pid, address, data, flags) {
      const m = new addon.Memory(new addon.Process(pid));
      return m.writeData(address, data, data.length, flags);
    },
    memory_createCache(pid, address, size, blockSize, maxBlocks, flags) {
      return new addon.Memory(new addon.Process(pid)).createCache(address, size, blockSize, maxBlocks, flags);
    },
    memory_clearCache(pid) {
      new addon.Memory(new addon.Process(pid)).clearCache();
    },
    memory_deleteCache(pid) {
      new addon.Memory(new addon.Process(pid)).deleteCache();
    },
    memory_isCaching(pid) {
      return new addon.Memory(new addon.Process(pid)).isCaching();
    },
    memory_getCacheSize(pid) {
      return new addon.Memory(new addon.Process(pid)).getCacheSize();
    }
  };
}
